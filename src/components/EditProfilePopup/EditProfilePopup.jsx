...
export default function EditProfilePopup({isOpen, onClose, onUpdateUser, isLoaderMessage}) {
    const currentUser = useContext(CurrentUserContext)
    const { values, errors, isValid, handleChange, reset, setValue} = UseFormValidation()
    ...
    return (
        ...
        />
        <span id="error-name" className="popup__error-message">{errors.username}</span>
        <input
            type="text"
            required
            minLength={2}
            ...
            onChange={handleChange}
        />
        <span id="error-hobby" className="popup__error-message">{errors.hobby}</span>
        </PopupWithForm>
    )
}